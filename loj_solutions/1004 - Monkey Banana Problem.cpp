...
int grid[210][210], dp[210][210], n;
int soln(int row, int col)
{
    if(row >= 2*n)
        return 0;
    if(col > 2*n-row || col <= 0)
        return 0;
   
    //~ deb(row, col, "---", grid[row][col]);
       
    if(dp[row][col] == -1)
    {
        if(row < n)
            dp[row][col] = max( soln(row+1, col), soln(row+1, col+1) ) + grid[row][col];
           
        else
            dp[row][col] = max( soln(row+1, col-1), soln(row+1, col) ) + grid[row][col];
    }
    //~ deb(dp[row][col]);
